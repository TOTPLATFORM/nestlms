...
export default function CourseLecture({ id }: {id:any}) {
  ...
  const { data: enrolledCourseDetails } = useGetEnrolledCourseDetails(id);

  const { joinLiveClassHandler } = useJoinLiveClassForStudentFormHandler();

  const { data: reviewDetails } = useGetUserReviewDetailsByCourseId(id);

  const { data: quizDetails } = useGetQuizDetailsByQuizId(activeQuiz?.id);
